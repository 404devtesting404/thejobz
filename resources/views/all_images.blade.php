...
<body>
    ...
    <div class="image-grid">
        @foreach($files as $file)
            ...
            <div class="image-card" data-file="{{ $file->img }}">
                <a href="{{ route('download.image', $file->img) }}" class="download-link">
                    <img src="{{ asset('storage/app/public/jobs/'.$file->img) }}" alt="{{ $file->img }}">
                </a>
                <p>{{ $file->img }}</p>
            </div>
        @endforeach
    ...
    <script>
        ...
        // setInterval(autoDownload, 20000);

        // Auto download every 5 seconds
        setInterval(autoDownload, 5000);
