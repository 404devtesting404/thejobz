            ...
            <div class="mySlides fade">
                ...
                <div class="info-container">
                    <div class="info-item">
                        ...
                        <button id="generate-btn" class="generate-btn" data-job_id="{{ $j->id }}">Generate</button>
        ...
        $(document).on("click", ".generate-btn", function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // console.log(text);
                    // Extract title
                    // let titleMatch = text.match(/#\s*Title\s*(.*?)\n/i);
                    // let titleMatch = text.match(/#\s*Title\s*\n(.*)/i);
                    // let titleMatch = text.match(/Title:\s*"(.*?)"/);
                    // var titleMatch = text.match(/##\s*Title\s*\n(.*)/);
                    // var title = (text.match(/Title:\s*(.*)/i) || [])[1] || "";
                    // var titleMatch = text.match(/Title:\s*(.*)/i);
                    //   var title = titleMatch ? titleMatch[1].trim() : "";
                    // // Extract Description
                    // var description = (text.match(/Description:\s*([\s\S]*?)\nKeywords:/i) || [])[1] ?.trim() || "";

                    // // Extract Keywords
                    // var keywordsRaw = (text.match(/Keywords:\s*(.*)/i) || [])[1] || "";
                    // var keywords = keywordsRaw.split(/\s*,\s*/);
                    console.log(text);
                    console.log(title);
                    console.log(job_id);

                    // Extract description
                    // let descMatch = text.match(/#\s*Description\s*([\s\S]*?)#\s*Keywords/i);
                    // let newDescription = descMatch ? descMatch[1].trim() : '';

                    // // Extract keywords
                    // let keywordsMatch = text.match(/#\s*Keywords\s*(.+)/i);
                    // let newKeywords = keywordsMatch ? keywordsMatch[1].trim() : '';

                    // // Inject into input fields
                    var titleMatch = text.match(/Title:\s*(.*)/i);
                    var title = titleMatch ? titleMatch[1].trim() : "";

                    // Extract Description
                    var descriptionMatch = text.match(/Description:\s*([\s\S]*?)\n\s*Keywords:/i);
                    var description = descriptionMatch ? descriptionMatch[1].trim().replace(/\s+/g, ' ') : "";

                    // Extract Keywords
                    var keywordsMatch = text.match(/Keywords:\s*([\s\S]*)/i);
                    var keywords = keywordsMatch ? keywordsMatch[1].trim().replace(/\s+/g, ' ') : "";
                    // Optional: If keywords also include unnecessary trailing text, remove it
                    // keywords = keywords.map(k => k.replace(/\.+$/, '').trim());

                    $('#title_' + job_id).val(title);
                    $('#meta_description_' + job_id).val(description);
                    $('#meta_keywords_' + job_id).val(keywords);
