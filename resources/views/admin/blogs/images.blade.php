        ...
        document.addEventListener("click", async function(event) {
            if (event.target.classList.contains("head-btn")) {
                try {
                    ...
                    let contentToCopy = "Write me an SEO friendly content on this data. the articel should be human friendly, it should not seem like it is written by AI, and should be 100% unique and 0% plaglarims free, which get ranked in Google also. write only title Description and keyword \n\n"; // 🎯 Sabse pehle ye line likh raha hai
        ...
        document.addEventListener("click", async function(event) {
            if (event.target.classList.contains("fb-btn")) {
                try {
                    ...
                    const textToCopy = `${contentToCopy}\nTitle: ${title}\nDepartment: ${department}\nApply Now: ${fullUrl}`; 
